...
@section('main')
    <div class="container mx-auto my-32 max-w-3xl">
        ...
        <form id="stepperForm" method="POST" action="{{ url('/generate-token') }}">
            @csrf
            ...
            <div class="step step-2">
                ...
                <fieldset>
                    @foreach($services as $item)
                        <div class="flex items-center mb-4">
                            <input id="service-{{$item->id}}" type="radio" name="service" value="{{ $item->id }}" class="w-4 h-4 border-gray-300 focus:ring-2 focus:ring-blue-300 dark:focus:ring-blue-600 dark:focus:bg-blue-600 dark:bg-gray-700 dark:border-gray-600">
                            <label for="service-{{$item->id}}" class="block ms-2  text-sm font-medium text-gray-900 dark:text-gray-300">
                                {{ $item->name }}
                            </label>
                        </div>
                    @endforeach
                ...
                <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded">Generate Token</button>
            </div>
    ...
    <script type="module">
        $(document).ready(function() {
            ...
            function updateStepperIndicator(step) {
                $('.stepper li').removeClass('text-blue-600 dark:text-blue-500'); // Reset all steps
                $('.stepper li:lt(' + step + ')').addClass('text-blue-600 dark:text-blue-500'); // Highlight completed steps
            }
